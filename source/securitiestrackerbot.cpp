...
void SecuritiesTrackerBot::registerSignals()
{
    sigset_t signalSet;
...
void SecuritiesTrackerBot::makePidFile()
{
    QFile file;
    file.setFileName("/var/run/securitiestrackerbot.pid");
    file.open(QIODevice::Text | QIODevice::Truncate | QIODevice::WriteOnly);
    QTextStream textStream(&file);
    textStream<<QString::number(getpid());
    log("Ok", "PID " + QString::number(getpid()));
    file.close();
}

void SecuritiesTrackerBot::rmPidFile()
{
    QFile file;
    file.setFileName("/var/run/securitiestrackerbot.pid");
    file.remove();
...
void SecuritiesTrackerBot::controlBotMessage(TelegramBotUpdate update)
{
    ...
    if (message.chat.id == telegramMasterId && message.text == "?")
           controlBot->sendMessage(message.chat.id, " Версия 0.0.4 , запущен " + StartTime.toString(Qt::ISODate) +
    ...
    if (message.chat.id == telegramMasterId && message.text == "restart")
    {
           controlBot->sendMessage(message.chat.id, " Перезагрузка ...");
           QApplication::exit(0);
    }
}
...
void signalHandler(int signal)
{
    ...
    {
        case SIGHUP:
                                    SecuritiesTrackerBot::log("Ok", "Получен сигнал SIGHUP. Конфигурационный файл будет перечитан.");
                                    break;
        case SIGINT:
                                    SecuritiesTrackerBot::log("Ok", "Получен сигнал SIGINT. Приложение будет завершено.");
                                    QApplication::exit(0);
                                    break;
        case SIGTERM:
                                    SecuritiesTrackerBot::log("Ok", "Получен сигнал SIGTERM. Приложение будет завершено.");
                                    QApplication::exit(0);
                                    break;
        default:
                                    SecuritiesTrackerBot::log("Ok", " Получен сигнал №" + QString::number(signal) + ". Этот сигнал не имеет обработчика и будет проигнорирован.");
    }
